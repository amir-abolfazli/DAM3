...
class EvaluationVisualizer(BaseListener):
    ...
    def __init__(self, task_type, n_wait, dataset_name, metrics, n_models, model_names, data_dict):
        ...
        if constants.DATA_POINTS in metrics and n_models > 1:
            raise ValueError("Can not use multiple models with datasets points visualization.")
    ...
    def on_new_train_step(self, sample_id, data_buffer):
        ...
        be aware of some new datasets, the caller will invoke this function,
        passing the new datasets buffer.
        ...
        data_buffer: EvaluationDataBuffer
            A buffer containing evaluation datasets for a single training / visualization step.
    ...
    def __configure(self):
        ...
        The subplot size will self adjust to each screen size, so that datasets can
        ...
        for metric_id in self.metrics:
            ...
            if metric_id == constants.TRUE_VS_PREDICTED:
                ...
                sorted_data_ids.sort()    # For better usage of the color cycle, start with 'current' datasets
                ...
                if metric_id == constants.ACCURACY:
                ...
                elif metric_id == constants.BALANCED_ACCURACY:
                    plot_tracker.sub_plot_obj.set_title('Balanced Accuracy')
                    plot_tracker.sub_plot_obj.set_ylabel('bal_acc')
                elif metric_id == constants.GMEAN:
                    plot_tracker.sub_plot_obj.set_title('G-Mean')
                    plot_tracker.sub_plot_obj.set_ylabel('gmean')
                    ...
                    plot_tracker.sub_plot_obj.set_title('Sensitivity')
                    plot_tracker.sub_plot_obj.set_ylabel('sensitivity')
                elif metric_id == constants.RECALL:
                    plot_tracker.sub_plot_obj.set_title('Specificity')
                    plot_tracker.sub_plot_obj.set_ylabel('specificity')
...
class PlotDataTracker(object):
    """ A class to track relevant datasets for plots corresponding to selected metrics.
