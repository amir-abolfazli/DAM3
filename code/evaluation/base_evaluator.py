...
from metrics import WindowClassificationMeasurements, ClassificationMeasurements, \
    MultiTargetClassificationMeasurements, WindowMultiTargetClassificationMeasurements, \
...
class StreamEvaluator(BaseSKMObject, metaclass=ABCMeta):
    ...
    def __init__(self):
        ...
        self._diversity_buffer = None
        self._baseLearnerPerformance_buffer = None
        self._learnerWights_buffer = None
        self._add_removal_buffer = None

        self.model_types = []

        # self._diversity_numComponents = 1
    ...
    def _check_configuration(self):
        ...
        if self._output_type == constants.SINGLE_OUTPUT:
            ...
            if evaluation_metrics.union(classification_metrics) == classification_metrics or self.data_points_for_classification:
    ...
    def _init_metrics(self):
        ...
        # Evaluation datasets buffer
        self._data_dict = {}
        self._diversity_dict = {}
    ...
    def _update_metrics(self):
        """ Updates the metrics of interest. This function updates the evaluation datasets buffer
        ...
        If more than one model/learner is evaluated at once, datasets is stored as lists inside
        the buffer.
        ...
        # numberBaseLearners = self.model[0].n_estimators
        # numComparisons = (numberBaseLearners*(numberBaseLearners-1))/2
        ...
        for metric in self.metrics:
            ...
            values = [[], []]
            diversity_values = {}
            balancedAccuracy_values = {}
            learnerWeights_values = {}
            add_remove_values = {}
            comNames = []
            ...
            elif metric == constants.BALANCED_ACCURACY:
                for i in range(self.n_models):
                    values[0].append(self.mean_eval_measurements[i].get_bacc_score())
                    values[1].append(self.current_eval_measurements[i].get_bacc_score())
            ...
            elif metric == constants.SPECIFICITY:
                for i in range(self.n_models):
                    values[0].append(self.mean_eval_measurements[i].get_specificity())
                    values[1].append(self.current_eval_measurements[i].get_specificity())
...
#########################################################################################
    ...
    def evaluation_summary(self):
        ...
        for i in range(self.n_models):
            ...
            if constants.BALANCED_ACCURACY in self.metrics:
                print('{} - Balanced Accuracy     : {}'.format(
                    self.model_names[i],
                    self._data_buffer.get_data(metric_id=constants.BALANCED_ACCURACY, data_id=constants.MEAN)[i]))
            ...
            if constants.SPECIFICITY in self.metrics:
                print('{} - Specificity: {:.4f}'.format(
                    self.model_names[i],
                    self._data_buffer.get_data(metric_id=constants.SPECIFICITY, data_id=constants.MEAN)[i]))
